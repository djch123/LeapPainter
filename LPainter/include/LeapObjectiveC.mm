...
- (BOOL)isFlipped
{
    return _interfaceDevice->isFlipped();
}
...
//////////////////////////////////////////////////////////////////////////
//IMAGE
@implementation LeapImage
{
    Leap::Image *_interfaceImage;
}

- (id)initWithImage:(void *)image
{
    self = [super init];
    if(self){
        _interfaceImage = new Leap::Image(*(const Leap::Image *)image);
    }
    return self;
}
...
- (NSString *)description
{
    std::string str = _interfaceImage->toString();
    return [NSString stringWithCString:str.c_str() encoding:[NSString defaultCStringEncoding]];
}
- (void *)interfaceImage
{
    return (void *)_interfaceImage;
}
- (int32_t)id
{
    return _interfaceImage->id();
}
- (int64_t)sequenceId
{
    return _interfaceImage->sequenceId();
}

- (int)width
{
    return _interfaceImage->width();
}
- (int)height
{
    return _interfaceImage->height();
}
- (int)bytesPerPixel
{
    return _interfaceImage->bytesPerPixel();
}
- (LeapImageFormatType)format
{
    return (LeapImageFormatType)_interfaceImage->format();
}
- (int)distortionWidth
{
    return _interfaceImage->distortionWidth();
}
- (int)distortionHeight
{
    return _interfaceImage->distortionHeight();
}
- (float)rayOffsetX
{
    return _interfaceImage->rayOffsetX();
}
- (float)rayOffsetY
{
    return _interfaceImage->rayOffsetY();
}
- (float)rayScaleX
{
    return _interfaceImage->rayScaleX();
}
- (float)rayScaleY
{
    return _interfaceImage->rayScaleY();
}
- (const unsigned char *) data
{
    return _interfaceImage->data();
}
- (const float *) distortion
{
    return _interfaceImage->distortion();
}

- (LeapVector *)rectify:(LeapVector *)uv
{
    const Leap::Vector input([uv x], [uv y], [uv z]);
    const Leap::Vector rectified = _interfaceImage->rectify(input);
    return [[LeapVector alloc] initWithX:rectified.x y:rectified.y z:rectified.z];
}
- (LeapVector *)warp:(LeapVector *)xy
{
    const Leap::Vector input([xy x], [xy y], [xy z]);
    const Leap::Vector warped = _interfaceImage->warp(input);
    return [[LeapVector alloc] initWithX:warped.x y:warped.y z:warped.z];
}
- (BOOL)isValid
{
    return _interfaceImage->isValid();
}
- (BOOL)equals:(const LeapImage *)other
{
    return *_interfaceImage == *(const Leap::Image *)[other interfaceImage];
}
- (void)dealloc
{
    delete _interfaceImage;
}

+ (LeapImage *)invalid
{
    static const Leap::Image &invalid_image = Leap::Image::invalid();
    return [[LeapImage alloc] initWithImage:(void *)&invalid_image];
}
@end
...
- (id)initWithFrame:(const void *)frame
{
    ...
    if (self) {
        ...
        NSMutableArray *images_ar = [NSMutableArray array];
        for (int i = 0; i < leapFrame.images().count(); i++) {
            const Leap::Image &tmpLeapImage = leapFrame.images()[i];
            LeapImage *image = [[LeapImage alloc] initWithImage:(void *)&(tmpLeapImage)];
            [images_ar addObject:image];
        }
        _images = [NSArray arrayWithArray:images_ar];
    }
    return self;
}
...
- (NSData *)serialize
{
    return [NSData dataWithBytes:self->_interfaceFrame->serialize().data() length:self->_interfaceFrame->serializeLength()];
}

+ (LeapFrame *)deserialize:(NSData *)serializedFrame
{
    const unsigned char *dataStr = (const unsigned char*)[serializedFrame bytes];
    Leap::Frame temp = Leap::Frame::invalid();
    temp.deserialize(dataStr, (int)serializedFrame.length);
    LeapFrame *frame = [[LeapFrame alloc] initWithFrame:(void *)&temp];
    return frame;
}
...
public:
    ...
    virtual void onImages(const Leap::Controller& leapController)
    {
        @autoreleasepool {
            [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadName:@"OnImages" object:_controller];
        }
    }
...
public:
    ...
    {
        @autoreleasepool {
            if ([_delegate respondsToSelector:@selector(onImages:)]) {
                [_delegate onImages:_controller];
            }
        }
    }
...
- (id)initWithListener:(id<LeapListener>)leapListener
{
    ...
    if (self) {
        ...
        if ([leapListener respondsToSelector:@selector(onImages:)]) {
            [nc addObserver:leapListener selector:@selector(onImages:) name:@"OnImages" object:self];
        }
...
{
    _controller->setPolicy((Leap::Controller::PolicyFlag)policy);
}

- (void)clearPolicy:(LeapPolicyFlag)policy
{
    _controller->clearPolicy((Leap::Controller::PolicyFlag)policy);
}

- (BOOL)isPolicySet:(LeapPolicyFlag)policy
{
    return (BOOL)_controller->isPolicySet((Leap::Controller::PolicyFlag)policy);
}
...
- (BOOL)addListener:(id<LeapListener>)listener
{
    ...
    if ([listener respondsToSelector:@selector(onImages:)]) {
        [nc addObserver:listener selector:@selector(onImages:) name:@"OnImages" object:self];
    }
...
- (NSArray *)images
{
    Leap::ImageList leapImages = _controller->images();
    NSMutableArray *images_ar = [NSMutableArray array];
    for (Leap::ImageList::const_iterator it = leapImages.begin(); it != leapImages.end(); ++it) {
        Leap::Image leapImage = *it;
        LeapImage *image = [[LeapImage alloc] initWithImage:(void *)&leapImage];
        [images_ar addObject:image];
    }
    return [NSArray arrayWithArray:images_ar];
}
